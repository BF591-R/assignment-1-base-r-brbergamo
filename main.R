...
summarize_matrix <- function(x, na.rm=FALSE) {
  x_means <-  apply(x, MARGIN = 1, FUN = mean, na.rm=na.rm)
  x_stdev <- apply(x, MARGIN = 1, FUN = sd,na.rm=na.rm)
  x_median <- apply(x, MARGIN = 1, FUN = median,na.rm=na.rm)
  x_min <- apply(x, MARGIN = 1, FUN = min, na.rm=na.rm)
  x_max <- apply(x, MARGIN = 1, FUN = max, na.rm=na.rm)
  x_num_lt_0 <-rowSums(x<0, na.rm=na.rm)
  x_num_btw_1_and_5 <- rowSums(x>1 & x<5, na.rm=na.rm)
  if (na.rm == TRUE)
  {
    x_num_na <- rep(0, nrow(x))
  }
  else
  {
    x_num_na <- rowSums(is.na(x))
  }
  df <- data.frame(mean=x_means, stdev=x_stdev, 
             median=x_median,min=x_min,
             max=x_max,num_lt_0=x_num_lt_0,
